          ...
          {work.items.map((item, i) => (
            <Reveal key={item.seed} variant="up" delay={(i % 3) * 0.06}>
              <figure className="group">
                <div className="relative aspect-[4/3] overflow-hidden rounded-[var(--d-radius-lg)] ring-1 ring-[var(--d-line)] transition-shadow duration-300 group-hover:shadow-[0_22px_50px_rgba(15,41,51,0.14)]">
                  <Image
                  ...
                  <span className="absolute left-3 top-3 rounded-full bg-[var(--d-bg)]/92 px-3 py-1 text-[11px] font-semibold uppercase tracking-wide text-[color:var(--d-accent-strong)] backdrop-blur">
                    {item.tag}
                  </span>
                </div>
                <figcaption className={`${display} mt-3 text-[15px] font-bold text-[color:var(--d-ink)]`}>
                  {item.title}
                </figcaption>
              </figure>
            </Reveal>
