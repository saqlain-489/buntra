        ...
        <div className="mt-12 grid gap-5 md:grid-cols-3">
          {testimonials.items.map((t, i) => (
            <Reveal key={t.name} variant="up" delay={i * 0.08}>
              <figure className="group flex h-full flex-col rounded-[var(--d-radius-lg)] bg-[var(--d-surface)] p-7 ring-1 ring-[var(--d-line)] transition-all duration-300 hover:-translate-y-1 hover:ring-[var(--d-accent)] hover:shadow-[0_22px_50px_rgba(15,41,51,0.10)]">
                <Quotes size={28} weight="fill" className="text-[color:var(--d-accent)]/40" />
                <blockquote className="mt-4 flex-1 text-[15px] leading-relaxed text-[color:var(--d-ink)]">{t.quote}</blockquote>
