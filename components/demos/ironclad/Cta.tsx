...
export function Cta() {
  return (
    <section id="assessment" className="bg-[var(--d-bg)] py-20 md:py-28">
      <DContainer>
            ...
            <div className="relative">
              ...
              <div className="mt-9 flex flex-col items-center justify-center gap-4 sm:flex-row">
                <Link
                  ...
                  className="inline-flex items-center justify-center gap-2 rounded-[var(--d-radius)] bg-[var(--d-accent)] px-7 py-4 text-[15px] font-semibold text-white transition hover:-translate-y-0.5 hover:brightness-110"
                >
                  {cta.primary.label} <ArrowRight size={17} weight="bold" />
                </Link>
                <a
                  href={`tel:${brand.phone.replace(/\D/g, '')}`}
                  className={`${mono} inline-flex items-center gap-2 text-[14px] font-medium text-white/85 transition hover:text-white`}
                >
